...
class Encoder(nn.Module):
    def __init__(self, in_channels, channels, n_embeddings, embedding_dim, jitter=0.5):
...
def test_encoder():
    n_mels = 80
    seq_len = 99
    mels = torch.rand((32, seq_len, n_mels))
    encoder = Encoder(n_mels, 128, 512, 64, 0.5)
    z, _, _ = encoder(mels.transpose(1, 2))
